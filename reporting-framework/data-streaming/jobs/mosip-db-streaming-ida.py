#-- -----------------------------------------------------------------------------------------
#-- Job Name        : IDA Transaction DB Streaming
#-- DB Name 	    : IDA DB
...
if __name__ == "__main__":
    sc = SparkContext(appName="KafkaStreamFromIdaDB")
    ...
    indexname = config.get("ElasticSearchSection", "ida-indexname")     
    ...
    def sendRecord(rdd):
        ...
        for element in list_elements:
            ...
            record['date'] = convertEpochDate(record['request_dtimes'])

            auth_req_dtimes = convertEpochDateTime(record['request_dtimes'])
            auth_res_dtimes = convertEpochDateTime(record['response_dtimes'])
            auth_cr_dtimes = convertEpochDateTime(record['cr_dtimes'])
            auth_upd_dtimes = convertEpochDateTime(record['upd_dtimes'])
            auth_del_dtimes = convertEpochDateTime(record['del_dtimes'])

            record['request_dtimes'] = auth_req_dtimes
            record['response_dtimes'] = auth_res_dtimes
            record['cr_dtimes'] = auth_cr_dtimes
            record['upd_dtimes'] = auth_upd_dtimes
            record['del_dtimes'] = auth_del_dtimes

            if record['status_code'] == 'Y':
                record['status_code'] = 'Success'
            elif record['status_code'] == 'N':
